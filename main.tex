...
\begin{equation*}
    \begin{aligned}
        {m_{i}}^{+}(t+1)\ =\ k_{m}\frac{K^{n}}{K^{n}+{P_{j}(t)}^{n}}+{k_{m}}_{0} \\[0.2cm]
        {P_{i}}^{+}(t+1)\ =\ k_{p}*{m_{i}(t)} \\[0.4cm]
        {m_{i}}^{-}(t+1)\ =\ k_{dm}*{m_{i}(t)} \\[0.2cm]
        {P_{i}}^{-}(t+1)\ =\ k_{dp}*{P_{i}(t)} \\[0.6cm]
        m_{i}(t+1)\ =\ {m_{i}}^{+}(t+1) - {m_{i}}^{-}(t+1) \\[0.2cm]
        P_{i}(t+1)\ =\ {P_{i}}^{+}(t+1) - {P_{i}}^{-}(t+1)
    \end{aligned}
    \hspace{0.8cm}
    \Bigg(\ 
        \begin{aligned}
...
\begin{conditions}
    ...
    P_{i,j}   &   number of protein molecules \\
    m_{i}   &   number of mRNA molecules\\
...
\begin{equation*}
    P_{lacI} \dashv P_{tetR}
\end{equation*}

\noindent where we are concerned only with two out of the six ODEs describing the Repressilator ($m_{i}\ \&\ P_{i};\ i\ =\ \textrm{\textit{tetR}},\ j\ =\ \textrm{\textit{lacI}}$). Assuming constant levels of LacI, we can examine the steady-state (once the system has reached it) concentration of TetR under different parameters --- namely, the Hill coefficient.

\begin{figure}
    \singlespacing
    \centering
    \includegraphics[width=0.75\textwidth]{suplementary_information_and_code/Task1_figure1.png}
    \caption{\textbf{Characteristics of repression kinetics.} }
    \label{fig:fig3}
\end{figure}
